...
押忍。
このプロジェクトは自律型AIエージェントの道場プロジェクトです。[Claude Code](https://github.com/anthropics/claude-code)を中心に設計しているため、他のAIエージェントでは動作しない可能性があります。
...
```bash
/plugin marketplace add bluestar-jp/bluestar-agent-dojo
```

### プラグインをインストール

```bash
/plugin install bluestar-agent-dojo@bluestar-dojo
```
...
```bash
/agents > deshi-skill-expert
/agents > deshi-code-reviewer
```
