...
**❌ BAD: 宣言から始める**
「本段落では、React のメモ化について解説していきます。まず最初に...」

**✅ GOOD: 直接内容（問題提起や結論）から始める**
「React アプリケーションのパフォーマンスが低下する主な原因は、不要な再レンダリングだ。これを防ぐ強力な武器がメモ化である。」

#### 2. 安全クッションの排除

**❌ BAD: 曖昧な表現で濁す**
「一般的に、メモ化を使用することで速度が向上する可能性があると考えられます。」

**✅ GOOD: 明確な条件と断定**
「メモ化を適切に適用すれば、再レンダリングコストを 80% 削減できる。ただし、すべてのコンポーネントに適用するのは避けるべきだ。」
