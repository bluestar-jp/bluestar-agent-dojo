...
すべてのエージェント定義ファイルは以下のYAMLフロントマターで開始すること。

```yaml
---
name: [agent-name]
description: [説明]
model: sonnet|opus|haiku    # 必須: 実行モデルの指定
tools: [ツール一覧]
skills:                     # 任意: 依存スキル
  - [skill-name]
---
...
エージェントの役割に応じて適切なモデルを選定すること。

| モデル | 用途 | 適用例 |
| -------- | ------ | -------- |
| `opus` | 戦略策定、計画立案、複雑な判断 | shihan系オーケストレーター、差分分析、レビュー戦略策定 |
| `sonnet` | 専門タスク実行、レビュー、執筆 | deshi系専門エージェント、コードレビュー、テスト実行 |
| `haiku` | 軽量な操作、定型処理 | git操作、フォーマット変換、簡単な判定 |
