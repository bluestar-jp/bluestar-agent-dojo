...
---
## エージェント定義におけるフロントマターの重要性

エージェントの行動と性能を最適化し、役割を明確にするため、フロントマターの適切な設定は不可欠です。

### モデル指定 (`model`) の基準

エージェントの役割と複雑性に応じて、最適なモデルを選択してください。

- **`deshi-*` エージェント (専門型)**: `sonnet` モデルを推奨。効率性とコストバランスに優れます。
- **`shihan-*` エージェント (統合・オーケストレーション型)**: `opus` モデルを推奨。複雑な判断と高度な連携能力が求められるためです。

### 連携スキル (`skills`) の明示

- **目的**: エージェントが連携するスキルを明示的に宣言し、エージェントの機能と依存関係を明確にします。
- **記述**: 連携するスキルの`name`をリスト形式で記載してください。参照するスキルがない場合、この項目は不要です。
